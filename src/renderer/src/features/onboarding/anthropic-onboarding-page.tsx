...
interface AnthropicOnboardingPageProps {
  onBack?: () => void
  onComplete?: () => void
}

export function AnthropicOnboardingPage({ onBack, onComplete }: AnthropicOnboardingPageProps) {
  ...
  // Mock existing token state - replace with actual implementation later
  const [hasExistingToken] = useState(false)
  const [existingToken] = useState<string | null>(null)
  const [checkedExistingToken] = useState(true)
  ...
  // Removed auto-start - now only starts when user clicks "Connect"

  // Mock polling for OAuth URL - replace with actual implementation later
  useEffect(() => {
    if (flowState.step === "waiting_url") {
      const timeout = setTimeout(() => {
        const mockOauthUrl = "https://console.anthropic.com/mock-oauth"
        setSavedOauthUrl(mockOauthUrl)
        setFlowState({
          step: "has_url",
  ...
  useEffect(() => {
    if (
      ...
      setUrlOpened(true)
      // TODO: Use Electron's shell.openExternal to open flowState.oauthUrl
      console.log("Opening OAuth URL:", flowState.oauthUrl)
    }
  ...
  const handleConnectClick = async () => {
    ...
    if (flowState.step === "idle") {
      // Start auth flow when user clicks Connect
      setFlowState({ step: "starting" })

      // TODO: Replace with actual API call: POST /api/auth/start
      // Mock: Simulate backend creating OAuth session
      setTimeout(() => {
        setFlowState({
          step: "waiting_url",
          sandboxId: "mock-sandbox-id",
          sandboxUrl: "mock-sandbox-url",
      ...
      // Mock retry - replace with actual API call later
      setTimeout(() => {
        setFlowState({
          step: "waiting_url",
          sandboxId: "mock-sandbox-id",
  ...
  const handleUseExistingToken = async () => {
    ...
    // Mock token import - replace with actual API call later
    setTimeout(() => {
      onComplete?.()
    }, 1000)
  }
  ...
  const submitCode = async (code: string) => {
    ...
    // Mock code submission - replace with actual API call later
    setTimeout(() => {
      // Simulate success
      onComplete?.()
    }, 1500)
  ...
  const handleOpenFallbackUrl = () => {
    if (savedOauthUrl) {
      // TODO: Open URL via Electron
      console.log("Opening fallback URL:", savedOauthUrl)
    }
  }
